...
const StudentData = () => {
  const [students, setStudents] = useState([
    ...
    { id: 3, name: "ghi", roll: "3", marks: { JAVA: 55, PHP: 10, JAVASCRIPT: 30 } },
  ]);

  const [formData, setFormData] = useState({
    id: null, name: "", roll: "", JAVA: "", PHP: "", JAVASCRIPT: ""
  });
  ...
  const handleSubmit = (e) => {
    ...
    setFormData({ id: null, name: "", roll: "", JAVA: "", PHP: "", JAVASCRIPT: "" });
  };
      ...
      <div className="card p-3 mt-4">
        ...
        <form className="row g-2" onSubmit={handleSubmit}>
          <div className="col-md-2">
            <input type="text" className="form-control" name="roll" placeholder="Roll" value={formData.roll} onChange={handleChange} required />
          </div>
          <div className="col-md-3">
            <input type="text" className="form-control" name="name" placeholder="Name" value={formData.name} onChange={handleChange} required />
          </div>
          <div className="col-md-2">
            <input type="number" className="form-control" name="JAVA" placeholder="JAVA" value={formData.JAVA} onChange={handleChange} required />
          </div>
          <div className="col-md-2">
            <input type="number" className="form-control" name="PHP" placeholder="PHP" value={formData.PHP} onChange={handleChange} required />
          </div>
          <div className="col-md-2">
            <input type="number" className="form-control" name="JAVASCRIPT" placeholder="JS" value={formData.JAVASCRIPT} onChange={handleChange} required />
          </div>
          <div className="col-md-1">
            <button type="submit" className="btn btn-success w-100">{isEdit ? "Update" : "Add"}</button>
          </div>
        </form>
      </div>
          ...
          {students.map((s) => {
            ...
            return (
              <tr key={s.id}>
                ...
                <td>{total}</td>
                <td className={result === "Pass" ? "text-success" : "text-danger"}>
                  {result}
                </td>
                <td>
